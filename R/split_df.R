...
#' @param num_col An integer specifying the number of columns to split the data frame into.
...
#' @export
#' @md
split_df <- function(df, num_col, return_list = FALSE) {
  ncols <- ncol(df)
  nrows <- nrow(df)

  if (num_col <= 0) {
    stop("num_col must be a positive integer.")
  }
  ...
  df_list <- vector("list", num_col)  # Preallocate list for efficiency
  colname_list <- vector("list", num_col)
  for (i in seq_len(num_col)) {
    ...
    if (nrow(df_part) < rows_per_part) {
      df_part <- rbind(df_part, as.data.frame(matrix(NA, nrow = rows_per_part - nrow(df_part), ncol = ncols)))
    }
